...
Plugin URI: http://1uthavi.adadaa.com/ada-feedwordpress-keyword-filters/
...
class AdaFWPKeyFilters {
	...
	if ($page->for_feed_settings()) :
		$ada_key_filters = maybe_unserialize($page->link->settings['ada key filters']);
		$include_sitewide_ada_key_filters = $page->link->settings['include sitewide ada key filters'];
...
/*CAPitalZ{ TEST manually
			$feed = get_syndication_feed_object();
			if (is_object($feed)) {
				$disable_ada_key_filters = $feed->setting('disable ada key filters', 'feedwordpress_disable_ada_key_filters', 'yes');
			
				if($disable_ada_key_filters != 'yes')
					$meta = $feed->setting('ada key filters', 'feedwordpress_ada_key_filters', false);
			}
			else {
				$disable_ada_key_filters = 'yes';
			}
			
		$post['post_title'] = "I do";
		$post['post_content'] = "I support free Tamileelam";
		
		
		echo '$post[title]=' . $post['post_title'] . '<br />';
		echo '$post[post_content]=' . $post['post_content'] . '<br />';
		
		echo $disable_ada_key_filters . '=disable_ada_key_filters<br />';
		echo (is_array($meta)) ? 'keywords are in good form [ARRAY]<br />' : 'keywords are not in good form [NOT ARRAY]<br />';
	   if($disable_ada_key_filters != 'yes' && is_array($meta)){
		if(!$this->ada_include($post,$meta))
			echo '***NOT INCLUDE***<br />';
		else
			echo '***INCLUDE***<br />';
	   }
	...
	foreach ($ada_key_filters as $index => $line) :
		...
		?>
<li id="ada-key-filter-<?php print $index; ?>-li" class="<?php print implode(' ', $line['class']); ?>">&raquo; <strong>Add</strong>
  <select id="ada-key-filter-<?php print $index; ?>-logic" name="ada_key_filters[<?php print $index; ?>][logic]" style="width: 8.0em">
    <option value="OR"<?php print $selected['OR']; ?>>OR</option>
    <option value="AND"<?php print $selected['AND']; ?>>AND</option>
    <option value="ORNOT"<?php print $selected['ORNOT']; ?>>ORNOT</option>
    <option value="ANDNOT"<?php print $selected['ANDNOT']; ?>>ANDNOT</option>
  </select>
  of <?php print $syndicatedPosts; ?>:
  <textarea style="vertical-align: top" rows="2" cols="30" class="ada-key-filter-keywords" id="ada-key-filter-<?php print $index; ?>-keywords" name="ada_key_filters[<?php print $index; ?>][keywords]"><?php print htmlspecialchars($keywords); ?></textarea>
</li>
<?php
		endif;
	endforeach;
...
function ada_include($post,$meta){
		...
		if($this->debug) { //echo "Checking *POST CONTENT*<br />";
	...
	if(is_null($include)) {
		if($this->ada_is_found_all($meta, "NOT")) {	//is only the NOT logic found?
			...
			if($this->debug) { //echo "Could not find any keywords && all NOT logic, hence INCLUDE.<br />";
			...
			if($this->debug) { //echo "Could not find any keywords, hence NOT INCLUDE.<br />";
...
function ada_include_content($text,$meta,$include = null){	//$include = the result of step 1; checked against post title
  ...
  if(is_array($meta))
	foreach ($meta as $rule) {
		...
		if($this->debug) { //echo "Checking " . $rule['logic'] . ": '" . implode(',', $keywords) . "'<br />";
		...
		if($is_found) {
			if($this->debug) { //echo $rule['logic'] . ": '" . implode(',', $keywords) . "' found.<br />";
	...
	if ( is_array( $posts )) {
		foreach ( $posts as $key => $item ) {
			...
			if(!$this->ada_include($post->post,$meta)) {		
				if($this->debug) { //echo '***NOT INCLUDE***<br />';
					fprintf($this->handle, "***NOT INCLUDE***\n");
				}
				unset( $posts[$key] );
			}		
			else if ( $this->debug ) { //echo '***INCLUDE***<br />';
